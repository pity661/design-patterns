...
public class TestMain {
  ...
  public static void copyProperties(Object source, Object target) throws Exception {
    ...
    Arrays.asList(sourceProperty).stream()
        .forEach(
            sourceSingle -> {
              Arrays.asList(targetProperty).stream()
                  .forEach(
                      targetSingle -> {
                        if (sourceSingle.getName().equals(targetSingle.getName())
                          ...
                          try {
                            ...
                            targetSingle
                                .getWriteMethod()
                                .invoke(target, sourceSingle.getReadMethod().invoke(source));
  ...
  // 拷贝两个对象除指定属性外的其余属性
  public static void copyPropertiesExcept(Object source, Object target, String... properties)
      throws Exception {
    // 获取对象的BeanInfo信息
    BeanInfo sourceBean = Introspector.getBeanInfo(source.getClass(), Object.class);
    // 获取属性的描述器
              ...
              Arrays.asList(targetProperty).stream()
                  .forEach(
                      targetSingle -> {
                        if (sourceSingle.getName().equals(targetSingle.getName())
                            && sourceSingle.getPropertyType() == targetSingle.getPropertyType()
                            && !exceptNames.contains(sourceSingle.getName())) {
                          try {
                            // 通过反射机制来调用getter/setter方法
                            targetSingle
                                .getWriteMethod()
  ...
  // 拷贝两个对象指定的属性
  public static void copyPropertiesContain(Object source, Object target, String... properties)
      throws Exception {
    // 获取对象的BeanInfo信息
    BeanInfo sourceBean = Introspector.getBeanInfo(source.getClass(), Object.class);
    // 获取属性的描述器
