...
public class TwoLockSingleton {
    ...
    /**
     * 进阶:  instance = new TwoLockSingleton() 是非原子性操作
     * 1. 给instance分配内存
     * 2. 调用TwoLockSingleton构造方法完成初始化操作
     * 3.使instance对象的引用指向分配的内存空间(完成这一步instance才不是null)
     *
     * 原因:
     * JVM会对指令进行重排序优化，可能3在2之前执行了，3执行完之后instance就不是null了
     * 如此时另外一个线程访问该方法就会直接返回 instance
     *
     * 解决方法:
     * jdk 1.5之后才会有效
     * 将instance变量声明为 volatile
     */
}
